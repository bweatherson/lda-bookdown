...
kable(category_table,  
	col.names = c("Topic", "Category", "Subject", "Keywords"))
```
...
```{r category-count, fig.cap = "Number of articles in each of the twelve categories over time"}
...
```{r category-count-trends, fig.cap = "Number of articles in each of the twelve categories over time (with trendlines)"}
category_count_graph + 
...
```{r category-weight-graph,  fig.cap = "Weighted number of articles in each of the twelve categories over time"}
...
```{r category-weight-graph-trendlines, fig.cap = "Weighted number of articles in each of the twelve categories over time (with trendlines)"}
category_weight_graph + 
  geom_point(size = 0.5, alpha = 0.2) +
  geom_smooth(se=F, 
              method = "loess", 
...
```{r category-frequency-graph, fig.cap = "Proportion of articles in each Category each year"}
...
```{r category-frequency-graph-trendlines, fig.cap = "Proportion of articles in each Category each year (with trendlines)"}
...
```{r category-frequency-graph-facet, fig.cap = "Proportion of articles in each Category (with facets)"}
...
# Tabel of Data Points that are excluded by the topic graph
...
print(
     kable(filter(category_by_topic, category == cate, f > 0.0601, year > 1900) %>%
          select(name, year, f) %>%
          mutate(f= round(f, 4)),
        caption = paste0("Points excluded from topic graph for ",cate),
...
The sub-topic "OLP Ethics". The "OLP" here is for ordinary language philosophy. So this subtopic is the ethics part of ordinary language philosophy. I'm moderately confident that this really is part of ethics - it includes papers like Philippa Foot's [Moral Beliefs](https://philpapers.org/rec/FOOVB). But I'm not 100% sure about this.
...
```{r lm-topics, fig.cap = "Topics in Logic and Mathematics"}
...
```{r metaphysics-and-idealism-frequency, fig.cap = "Proportion of each year's articles that are in Idealism or Metaphysics", fig.height = 5}
cate_num <- 7
...
```{r metaphysics-and-idealism-count, fig.cap = "Weighted number of each year's articles that are in Idealism or Metaphysics", fig.height = 5}
...
The Psychology topic, which I had always associated with _Mind_ pre-1900, hangs around into the 1930s. It doesn't make up a big portion of what _Mind_ does in the 20th century, but it is an important part of the early years of _Philosophical Review_. Some of the other topics are arguably papers that could just have easily be classified with Psychology. That's true for the handful of Color papers in the 1880s and the pre-1900 work on Emotions. It's also somewhat true of the early work on Self-Consciousness, though some of that work is also idealist influenced.
