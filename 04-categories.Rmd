...
Next, we'll look at how many articles are in each category in each year. I'll say an article is 'in' a category if the probability the model gives to the article being from that category is greater than the probability it gives to the article being from any other category. And the probability an article is from a category is just the sum of the probability that it is from one of the topics (or subtopics) that make up the category. (What's a subtopic? I'll get to that in the next chapter.)
...
category_count_graph <- ggplot(category_count, aes(x = year, y = n, color=category, group=category)) +
    labs(x = "Year", y = "Number of Articles", title = "Number of Articles in Each Category Per Year")
...
```{r category_graph_setup, fig.height=12, fig.width = 10}
...
```{r category_weight_graph}
category_weight_graph <- ggplot(category_year, aes(x = year, y = y, color=category, group=category)) +
    labs(x = "Year", y = "Weighted Number of Articles", title = "Weighted Number of Articles in Each Category Per Year")

category_weight_graph + geom_point(size = 0.5, alpha = 0.5) + theme(legend.title = element_blank())
```
...
```{r category_weight_graph_trendlines}
category_weight_graph <- ggplot(category_year, aes(x = year, y = y, color=category, group=category)) +
    labs(x = "Year", y = "Weighted Number of Articles", title = "Weighted Number of Articles in Each Category Per Year")

category_weight_graph + geom_point(size = 0.5, alpha = 0.2) + geom_smooth(se=F, method = "loess", size = 0.2)  + theme(legend.title = element_blank())
```
...
category_frequency_graph + geom_point(size = 0.5, alpha = 0.2) + geom_smooth(se=F, method = "loess", size = 0.2)  + theme(legend.title = element_blank())
```
...
journal_denominator <- articles %>%
  select(file, journal, year) %>%
  group_by(journal, year) %>%
  dplyr::summarise(d = n_distinct(file))
...
ggplot(data = filter(category_by_journal, category == cate), aes(x = year, y = f))  +
      ...
      labs(x = "Year", y = "Weighted Proportion of Articles", title = paste0("Articles from ",cate," in each Journal")) +
...
```{r ethics_topics, fig.height=10.2}
ggplot(data = filter(category_by_topic, category == cate), aes(x = year, y = f))  +
      geom_point(size = 0.15, colour = hcl(h = (cate_num-1)*30+15, l = 65, c = 100)) +
      theme(legend.position="none") +
      labs(x = "Year", y = "Weighted Proportion of Articles", title = paste0("Topics from ",cate)) +
      facet_wrap(~topic, ncol = 3, labeller = labeller(topic = subtopic.labs))
```
...
```{r history_topics, fig.height= 6.2}
ggplot(data = filter(category_by_topic, category == cate), aes(x = year, y = f))  +
      geom_point(size = 0.15, colour = hcl(h = (cate_num-1)*30+15, l = 65, c = 100)) +
      theme(legend.position="none") +
      labs(x = "Year", y = "Weighted Proportion of Articles", title = paste0("Topics from ",cate)) +
...
```{r metaphysics_topics, fig.height = 6.2}
ggplot(data = filter(category_by_topic, category == cate), aes(x = year, y = f))  +
      geom_point(size = 0.15, colour = hcl(h = (cate_num-1)*30+15, l = 65, c = 100)) +
      theme(legend.position="none") +
      labs(x = "Year", y = "Weighted Proportion of Articles", title = paste0("Topics from ",cate)) +
...
```{r metaphysics_and_idealism_frequency}
...
m_and_i_graph <- ggplot(data = m_and_i_table, aes(x = year, y = f)) +
      ...
      theme(legend.position="none") +
      ylim(0, 0.4) +
      labs(x = "Year", y = "Weighted Proportion of Articles", title = paste0("Articles from Idealism or Metaphysics"))
  
m_and_i_graph
```

That's more like what I expected, though the post-positivism rise is much smaller than the pre-positivism fall. It obviously looks different if you use raw numbers.

```{r metaphysics_and_idealism_count}
...
m_and_i_count_graph <- ggplot(data = m_and_i_count_table, aes(x = year, y = f)) +
      geom_point(size = 0.5, colour = hcl(h = (cate_num-1)*30+15, l = 65, c = 100)) +
      theme(legend.position="none") +
      labs(x = "Year", y = "Weighted Number of Articles", title = paste0("Articles from Idealism or Metaphysics"))
...
```{r language_topics, fig.height = 6.2}
ggplot(data = filter(category_by_topic, category == cate), aes(x = year, y = f))  +
      geom_point(size = 0.15, colour = hcl(h = (cate_num-1)*30+15, l = 65, c = 100)) +
      theme(legend.position="none") +
      labs(x = "Year", y = "Weighted Proportion of Articles", title = paste0("Topics from ",cate)) +
...
The graph for Speech Acts is nice. You see first an Austin-inspired surge of work in the 1960s and 1970s, then a dip, then a resurgence (perhaps Langton inspired) in more recent times. That feels about right, but it's nice to see it confirmed.
...
The "Early Empirical" work stays being a big deal through the 1930s. I guess it is Moore's ascension to the _Mind_ editorship that has a big effect here, along with other journals that are not interested in this kind of work coming online. Some of the other topics are arguably papers that could just have easily be classified with "Early Empirical". That's true for the handful of Color papers in the 1880s and the pre-1900 work on Emotions. It's also somewhat true of the early work on Self-Consciousness, though some of that work is also idealist influenced.
...
```{r science_topics, fig.height= 12.2}
ggplot(data = filter(category_by_topic, category == cate), aes(x = year, y = f))  +
      geom_point(size = 0.15, colour = hcl(h = (cate_num-1)*30+15, l = 65, c = 100)) +
      theme(legend.position="none") +
      labs(x = "Year", y = "Weighted Proportion of Articles", title = paste0("Topics from ",cate)) +
