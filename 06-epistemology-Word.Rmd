...
Those aren't all epistemology articles, but some of them are. The Ludwig clearly is, and the Nathan, and plausibly several others. What about the 8 that just missed the cut, i.e., the 10 with a probability of being in epistemology just below 0.2.
...
alt_text <- "A scatterplot showing how many articles in Epistemology there are each year. There are almost none before 1945, and over 60/year in recent years, with a fairly linear trend between them. The full data are in Table D.1."
```
...
```{r epistemology-weighted-count-graph, fig.height=9, fig.width=7.5, fig.cap = "Weighted number of articles in each epistemology topic", fig.cap = alt_text}
...
```{r epistemology-facet-graph, fig.height=15.2, fig.width=7, fig.cap = "40 epistemology topics", fig.alt = alt_text}
