...
```{r epistemology_main_count_graph, fig.height=9, fig.width=7.5}
...
kable(epistemology_missing, col.names = c("Year", "Subject", "Count")) %>% kable_styling(full_width = F)
```
<br>
...
```{r epistemology_weighted_count_graph, fig.height=9, fig.width=7.5}
...
#  geom_smooth(se = F, method = "loess", size  = 0.3, alpha = 0.3) +
  ...
  labs(x = "Year", y = "Number of Articles", title = "Number of Articles in Each Subject Per Year") +
  scale_colour_discrete(labels = epistemology_subjects$subject) + 
  theme(legend.title = element_blank(), legend.position = "bottom")
...
<br>
```{r epistemology_gamma_missing_data}
epistemology_gamma_missing <- epistemology_yeargamma_postwar %>%
  filter(gamsum > 6.02) %>%
  inner_join(epistemology_subjects, by = "topic") %>%
  select(year, subject, count = gamsum)

kable(epistemology_gamma_missing, col.names = c("Year", "Subject", "Weighted Count")) %>% kable_styling(full_width = F)
```
<br>
...
```{r epistemology_fact_graph, fig.height=15.2, fig.width=7}
epistemology_yeargamma_postwar$topic = as.factor(epistemology_yeargamma_postwar$topic)
...
```{r epistemology_category_graph, fig.height=6}
...
```{r gettier_first, fig.height=5, out.width = '100%'}
...
```{r gettier_second, fig.height=5, out.width = '100%'}
jjj <- 6

cate_nam <- epistemology_subjects$subject[jjj]

ggplot(data = filter(epistemology_yeargamma_postwar, topic == jjj), aes(x = year, y = gamsum, color = topic)) +
  geom_point(size = 1.5, alpha = 1, colour = hcl(h = (jjj-1)*(9)+15, l = 65, c = 50)) +
  geom_smooth(se = F, 
              formula = "y ~ x", 
              size = 0.5, 
              method = "loess", 
...
```{r gettier_third, fig.height=5, out.width = '100%'}
jjj <- 6
...
ggplot(data = filter(epistemology_yeargamma_postwar, topic == jjj), aes(x = year, y = gamrat_all)) +
  ...
  geom_smooth(se = F, formula = "y ~ x", size = 0.5, method = "loess", colour = hcl(h = (jjj-1)*(9)+15, l = 65, c = 100)) +
  theme(legend.position="none") +
  labs(x = "Year", y = "Proportion of Philosophy Articles") +
...
```{r gettier_fourth, fig.height=5, out.width = '100%'}
jjj <- 6
...
ggplot(data = filter(epistemology_yeargamma_postwar, topic == jjj), aes(x = year, y = gamrat_local)) +
  ...
  geom_smooth(se = F, formula = "y ~ x", size = 0.5, method = "loess", colour = hcl(h = (jjj-1)*(9)+15, l = 65, c = 100)) +
  theme(legend.position="none") +
  labs(x = "Year", y = "Proportion of Epistemology Articles") +
  scale_y_continuous(labels = scales::percent)
```
