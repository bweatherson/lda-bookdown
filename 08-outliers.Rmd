...
- Look for weirdnesses
- A bit of a stress test
- Also tells us something about the model

Ideas for outlier tests

- The various things in weight_ratio_test.R
- The articles with the highest and lowest confidences
- The highest and lowest correlations; both pairwise and average over all others (also post-45 correlations)
- Word length by topic (Done)
- Word length over time (Done)
- The bump (Done)
...
ggplot(long_articles) +
  geom_bar(aes(year)) + 
  scale_x_binned() +
  labs(x = "Decade", y = "Number of articles at least 40 pages long")
```
...
ggplot(year_normality, aes(x = year, y = normality)) + 
  geom_point(size = 0.5) +
  labs(x = "Year", y = "Normality (i.e., inverse informativeness)")
```
...
```{r mid-century-graph-by-topic, fig.height = 12.2}
...
```{r mid-century-graph-by-ratio, fig.height = 12.2}
...
```{r late-century-atlantic-graph-topic, fig.height = 12.2}
...
```{r late-century-atlantic-graph-ratio, fig.height = 12.2}
...
I've been going back and forth a lot between two ways of measuring topic size: raw count and weighted count. The raw count is the number of articles such that the probability of being in that topic is higher than the probability of being in any other topic. The weighted count is the sum, across all articles, of the probability of being in that topic. Mathematically, it is the expected number of articles in the topic, though I'm not sure that the notion of expectation makes much sense here.

How do these notions relate? This section is about looking at that question from a few angles, and asking whether we can use the relationship between them to get something like a measure of how specialised different topics are.
...
counting_tibble <- counting_tibble %>%
  mutate(x = w_in_r/r_c, y = w_c/r_c, z = (w_c - w_in_r)/w_c)
...
I'm going to focus on topic 80, [Modality](#topic80). It's raw count is `r m`. That is, there are 370 articles that have a higher probability of being in Modality than being in any other topic. And its weighted count is `r round(mw,3)`. The ratio between them is `r round(m/mw,3)`. They're almost the same. But as you saw through chapter \@ref(all-90-topics), those numbers could come apart quite a bit for some topics. Here is a graph of this ratio for all 90 topics.

```{r counting-tibble-y-topic, fig.height=12.2}
...
I don't quite understand why [Beauty](#topic08) and [Crime and Punishment](#topic36) are at the top of this graph. Let's come back to those.

Instead we'll look at the relationship between raw count and weighted count from another direction. Go back to Modality, and think about the `r m` articles that make up its raw count. These articles have very different probabilities of being in Modality. Here are two articles from the `r m`.
...
Among the `r m` articles that make up the raw count for Modality, the average probability the model gives to them being in the topic is `r counting_tibble$x[80]`. (That's a little under the midpoint between the Bird and the Keyt articles, but not absurdly so.)

How typical is that? Is the average maximal topic probability always around 0.4, or does it vary by topic? It turns out we can do this same calculation for each topic. Here are the results. (Note that the scale does not start at 0.)

```{r counting-tibble-x-topic, fig.height=12.2}
...
```{r counting-tibble-x-prob, fig.height=12.2}
...
Intuitively, what's going on here is that we have a rough measure of specialisation. If you're talking about [Quantum Physics](#topic66) or [Evolutionary Biology](#topic82), then you're probably really talking about quantum physics or evolutionary biology. But lots of people use [Ordinary Language](#topic24) and discuss [Concepts](#topic78). So lots of articles that are about lots of different topics can end up looking like Ordinary Language or Concepts articles.

There is another way we can think about specialisation. The `r m` articles that make up the raw count of Modality contributed, on average, `r round(counting_tibble$x[80],3)` to its weighted count. So collectively, they contributed `r round(counting_tibble$w_in_r[80],3)`. Since the raw count was `r round(mw, 3)`, that left `r round(mw - counting_tibble$w_in_r[80],3)`. Or, put another way, `r round(counting_tibble$z[80],3)` of the weighted count was made up by articles that aren't included in the raw count. 

This contribution, though large, was built up out of many small pieces. The largest single contribution comes from this article.
...
But most contributions aren't that big. In fact, the average is under 1%. But there are nearly 32,000 of them, so they add up. In fact, they add up to about 61% of the weighted count for Modality. Is that 61% typical? Well, we can again look at the graph for all 90 topics.

```{r counting-tibble-z-topic, fig.height=12.2}
...
And 61% is somewhat middling, though there's so much variation that I'm not sure what I'd say is typical. Let's look at that one arranged by order.

```{r counting-tibble-z-ratio, fig.height=12.2}
...
And again, it's sort of a measure of specialisation. (Though this time I've put the specialised articles at the top of the graph.) The model doesn't give Quantum Physics much credit for articles that aren't squarely about quantum physics. But it gives Arguments lots of credit for articles that are not primarily about arguments.

Those last two measures feel like they are sort of measures of specialisation, so they should be correlated. And indeed they are.

```{r compare-specialisation-measures}
...
And our two measures of specialisation do sort of line up. There are some outliers - [Functions](#topic68) is above the line and Beauty is below it. So there is something a bit distinctive about Beauty, though I can't really tell what it is.

It isn't mathematically impossible to get dots well away from this trend-line, though you need some serious discontinuities in the model to get it. If we deleted a few of the articles where Arguments is just barely the most probable topic, we'd move its light blue dot further towards the top right corner. It's a little harder to get a sensible result where Beauty (or anything else) goes to the bottom left, but it would be possible if a lot of articles where it was the second or third most likely topic were deleted.

I had originally thought that the ratio of raw count to weighted count would be a kind of specialisation measure as well - after all Arguments is an outlier and its the least specialised topic there is. If you think these other two measures are specialisation measures, then it turns out you get a mixed verdict on whether this ratio is a specialisation measure too. Here is how it correlates with one of them.

```{r compare-specialisation-measures-fail}
# Comparing Second and Third
...
There is a trend there, but it's not a strong one. But the relationship to the third graph is really strong.

```{r compare-specialisation-measures-succeed}
# Comparing Second and Third
...
On the one hand, there's a tight correlation here. But I think this is an artifact of the mathematics; it isn't measuring anything real. A theory that puts Evolutionary Biology and Functions at opposite ends of the spectrum isn't really measuring specialisation. And Beauty isn't the most specialised topic in the model.

I think the better thing to say is that we did have two measures of specialisation, and they were a little different, but not that different. But ratio of raw count to weighted count is something else; it's measuring how much something gets talked about when it's not the centre of attention. As we see in Beauty, that value can be low even when articles that are about Beauty often get confused by the model for something else.