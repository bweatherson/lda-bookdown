...
If these three things are all sort of measures of specialisation, they should correlate reasonably well. And it turns out that two pairs of them do. Here is what it looks like if you graph the second and third (i.e., $p$ and $\frac{rp}{w}) against each other.
...
But while these two are in fact quite well correlated, the first measure $\frac{r}{w}$, is not rreally correlated with either. Here is the graph of it against $p$.
...
There is a trend there, but it's not a strong one. But the relationship to $\frac{rp}{w}$ is really strong.
...
I think the better thing to say is that we did have two measures of specialisation:

- Average topic probability of articles in the topic, i.e., $p$.
- Proportion of weighted count that is from articles in the topic, i.e., $\frac{rp}{w}$

These don't quite measure the same thing, but they aren't that different. But ratio of raw count to weighted count is something else; it's measuring how much something gets talked about when it's not the centre of attention. As we see in Beauty, that value can be low even when articles that are about Beauty often get confused by the model for something else.