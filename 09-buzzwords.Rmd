...
datatable(imprint_summary_DT, 
          ...
          rownames = FALSE,
          options = list(columnDefs = list(list(className = 'dt-left', targets = 0:2))),
          caption = "Weighted sum of topic probabilities for articles in Philosophers' Imprint 2019."
          )%>%
  formatSignif('g',3) %>%
  formatStyle(1:3,`text-align` = 'left')
```

How does morms get to 4.8 like that? Lots of small pieces it turns out.
...
datatable(imprint_norms_DT %>%
          ...
          caption = "Probability of being in topic 90 for articles in Philosophers' Imprint 2019."
          )%>%
  formatSignif('gamma',3) %>%
