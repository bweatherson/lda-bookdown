...
This is part of why I was happy to include [19th Century Psych](#topic01) as a philosophy of mind topic. It is a bit different to how we now do philosophy of mind. But it includes a lot of what Russell does in _The Analysis of Mind_. And that's definition a philosophy of mind book.
...
It really is confident about Russellian works, relatively speaking. Let's see what happens if we leave off 19th Century Psych.
...
kable(gg_high_gamma %>%
      ...
      col.names = c("Book", "Chapter", "Subject", "Probability"),
      digits = c(0, 0, 0, 4),
      caption = "10 chapters with highest topic probabilities (excluding 19th Century Psych)")
```
...
early_gamma <- relabeled_gamma %>%
  filter(document %in% early_articles$file)
...
```{r first-thirty-book-journal-compare-mean, fig.height = 5, fig.cap = "Average probability for first 30 topics in journals and books"}
ggplot(filter(book_journal_graph, topic < 31), aes(x = jm, y  = bm, color = topicfactor)) + 
  ...
  geom_text(aes(label=ifelse(topic == 1 | topic == 2 | topic == 3 | topic == 16 | topic == 24,as.character(subject),'')),hjust=1.1,vjust=0) +
  theme(legend.position = "none") +
...
#  geom_text(aes(label=ifelse(topic == 41 | topic == 52 ,as.character(subject),'')),hjust=-.01,vjust=1.4) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0.06, 2.5)))
```
...
ggplot(filter(book_journal_graph, topic < 91, topic > 60), aes(x = jm, y  = bm, color = topicfactor)) + 
  ...
  theme(legend.position = "none") +
  labs(x = "Average probabilty of topic in journals", y = "Average probability of topic in books") +
  geom_text(aes(label=ifelse(topic == 1 | topic == 47 ,as.character(subject),'')),hjust=-.02,vjust=-0.4) +
  geom_text(aes(label=ifelse(topic == 65 | topic == 83 ,as.character(subject),'')),hjust=-.01,vjust=1.4) +
  geom_text(aes(label=ifelse(topic == 60 | topic == 60 ,as.character(subject),'')),hjust=-0.03,vjust=-0.2) +
...
ggplot(filter(book_journal_graph, topic < 91, topic > 60), aes(x = jh, y  = bh, color = topicfactor)) + 
  ...
  theme(legend.position = "none") +
  labs(x = "Maximum probabilty of topic in journals", y = "Maximum probability of topic in books") +
  geom_text(aes(label=ifelse(topic == 74 | topic == 76 | topic == 83 ,as.character(subject),'')),hjust=-.02,vjust=-0.4) +
...
```{r bad-gamma-absolute, fig.cap = "Norms topic in the Bad LDA"}
jjj <- 90
...
<br>
```{r bad-gamma-pr-2004}
kable(bad_gamma %>% filter(year == 2004, journal == "Philosophical Review") %>% select(citation, gamma),
...
kable(bad_gamma %>% filter(year == 2010, journal == "Ethics") %>% select(citation, gamma), 
      col.names = c("Article", "Topic Probability"),
...
kable(bad_gamma %>% filter(year == 2011, journal == "British Journal for the Philosophy of Science") %>% select(citation, gamma), 
      col.names = c("Article", "Topic Probability"),
