...
```{r weight, echo=FALSE, message=FALSE, results='asis', fig.height=8, fig.width = 6.25}
  ...
  indiv_weight_graphs <- ggplot(filter(yeargamma, as.numeric(topic) <= cats/2), aes(x = year, y = freq, color = topic, group=topic)) +
  ...
  indiv_weight_graphs <- ggplot(filter(journalgamma, as.numeric(topic) > cats/2), 
                                aes(x = year,
                                    y = freq, 
                                    color = topic, 
                                    group=topic)) +
