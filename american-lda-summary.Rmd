...
load("2022-data/American-1930-1999-meta.RData")
load("~/Documents/lda-bookdown/american_lda.RData")
...
---
title: "`r params$cats` Category LDA for Journal of Philosophy and Philosophical Review"
date: "1930-1999"
...
```{r weight, echo=FALSE, message=FALSE, results='asis', fig.height=8, fig.width = 6.25}
  ...
  indiv_weight_graphs <- ggplot(filter(journalgamma, topic <= cats/2), aes(x = year, y = freq, color = topic, group=topic)) +
    geom_point(size = 0.5) +
  ...
  indiv_weight_graphs <- ggplot(filter(journalgamma, topic > cats/2), 
                                aes(x = year, y = freq, color = topic, group=topic)) +
    geom_point(size = 0.5) +
    facet_wrap(~topic, ncol = 3) +
    theme(legend.position="none") +
    labs(x = "Year",
...
```{r cats, echo=FALSE, message=FALSE, results='asis', fig.height=6, fig.width = 6.25}
  for (jjj in 1:cats){
    cat(" \n")
    cat("## Topic ", jjj, "\n")
