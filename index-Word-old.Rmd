...
knitr::opts_knit$set(eval.after = c("fig.cap", "fig.alt", "tab.cap"))
...
```{r flextable-for-article-probabilities}
...
flextable(temp_gamma) %>% 
  set_header_labels(values = list(
    subject = "Subject",
    gamma = "Probability"
    )) %>% 
  set_caption(temp_article$citation[1], autonum = autonum) %>% 
...
```{r flextable-for-author-articles}
author_flextable <- function(x, y){
flextable(relabeled_articles %>%
          filter(auth1 %in% x | auth2 %in% x | auth3 %in% x) %>%
          arrange(year) %>%
        mutate(gamma = round(gamma, 4)) %>%
         inner_join(the_categories, by = "topic") %>%
          select(year, citation, subject, gamma) %>% 
          rename(Year = year, Article = citation, Subject = subject, Probability = gamma)) %>% 
    set_caption(paste0("Articles with Author ",y), autonum = autonum) %>% 
    set_table_properties(layout = "autofit")
  } 
```
...
journals_summary <- articles %>%
  ...
  flextable(journals_summary) %>% 
    set_header_labels(journal = "Journal", fyear = "First Year", art = "Number of Articles") %>% 
    set_caption("The twelve journals that this book talks about", autonum = autonum) %>% 
    set_table_properties(layout = "autofit")
...
```{r first-facet-graph, fig.cap = "Proportion of articles in each category per year", dev = 'png', fig.alt = alt_text}
...
flextable(topic_three) %>% 
  set_header_labels(citation = "Article") %>% 
  set_caption("Top articles in topic 3 in example LDA", autonum = autonum) %>% 
  set_table_properties(layout = "autofit")
```
...
```{r analyse-dummy-lda-words, tab.cap = "Topic 3 Words"}
load("my_lda.RData")
my_beta <- tidy(my_lda, matrix = "beta")
...
flextable(topic_three_words) %>% 
  set_header_labels(term = "Words") %>% 
  set_table_properties(layout = "autofit")
```
...
flextable(t) %>% 
  set_header_labels(citation = "Article", subject = "Subject", wordcount = "Wordcount") %>% 
  set_caption(paste0("Articles in which the word \'",x,"\' appears most often"), autonum = autonum) %>% 
  set_table_properties(layout = "autofit")
}
```
...
```{r analyse-dummy-lda-words, tab.cap = "More topic 3 Words"}
load("my_lda.RData")
my_beta <- tidy(my_lda, matrix = "beta")
