...
relabeled_gamma <- merge(relabeled_gamma, articles, by.x="document", by.y="file") %>%
...
# Second, the highly cited list, which is actually fairly easy -though have to check it is up to date
 ...
 short_keywords <- tribble(
  ...
  high_cite_gamma <- merge(highly_cited, relabeled_articles, by.x = "file", by.y = "document") %>% arrange(desc(Cites))
...
word_year_count <- all_journals_tibble %>%
  inner_join(article_year_tibble, by = "filename") %>%
  group_by(year) %>%
...
word_year_frequency <- function(x){
  left_join(word_year_count, all_journals_tibble %>%
    filter(word == x) %>%
    left_join(article_year_tibble, by = "filename") %>%
    group_by(year) %>%
...
word_frequency_graphs <- function(x){
  ...
  scale_y_continuous(labels = function(x) ifelse(x > 0, paste0("1/",round(1/x,0)), 0)) +
...
journals_summary <- articles %>%
  ...
  dplyr::summarise(fyear = min(year), art = n_distinct(file))
...
To be written, once I write the book and figure out what the plan was.
...
t <- all_journals_tibble %>%
  filter(word == x) %>%
  inner_join(relabeled_articles, by = c("filename" = "document")) %>%
  arrange(-wordcount) %>%
  top_n(10, wordcount) %>%
  inner_join(the_categories, by = "topic") %>%
