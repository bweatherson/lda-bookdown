...
articles <- articles %>% 
  mutate(across(.cols=title:title, str_replace_all, "\\rm", "\\Rm")) %>% 
  mutate(across(.cols=citation:citation, str_replace_all, "\\rm", "\\Rm")) 
...
relabeled_articles <- relabeled_articles %>% 
  mutate(across(.cols=mcitation:mcitation, str_replace_all, "\\rm", "\\Rm")) 
...
word_year_count <- saveall_journals_tibble %>%
...
This book is published by [Maize Books](https://www.fulcrum.org/maizebooks), an imprint of [Michigan Publishing](https://www.publishing.umich.edu/). The official published version can be found at

> [https://doi.org/10.3998/mpub.12251719](https://doi.org/10.3998/mpub.12251719)

It's published as an ebook, with continuous scrolling rather than pages. So any citations should be to sections of the book, not pages. The sections (and subsections) are short, so this should still make it reasonably easy to cite.

That said, the version of the book you are reading here has more capacities than the official version. As I'll describe in more detail in the next section, there are a lot of things that the website can do that a book cannot do. Most notably, many of the tables in this version can be searched and sorted. But the Maize Books version is the official, stable, version of the book.
...
<!-- This book is primarily designed to be read in a browser, and some features won't work on an ebook. Most notably, the searchable tables have been replaced with screenshots of what the table looks like by default. In the web version, these are numbered, and searchable, tables. I've taken out the numbers here, but the numbers for the other tables are left consistent with the web version for consistency. -->
