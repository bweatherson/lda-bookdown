...
```{r t02b, fig.cap=paste(the_categories$subject[jjj], "Articles in Each Journal")}
source('topic_summary_facet_graph.R')
```

```{r t02c, fig.cap=the_categories$subject[jjj], fig.height= 5.2}
cat("<br>")
...
Now you might think that five articles out of 1600 is not a lot. And you'd be right. But it's a bit more than we see in other topics, so it seemed worth noting. There are several other articles by Indian philosophers, the most significant such philosopher being [Hiralal Haldar](https://philpapers.org/s/Hiralal%20Haldar). But the five listed above are the main ones that we would think of as _Indian Philosophy_.

Much of the story of Anglophone philosophy in the first half of the twentieth century involves disputes between two or more of the four schools mentioned in the first paragraph of Roy Wood Sellars's [A Correspondence Theory of Truth](https://philpapers.org/rec/SELACT): idealism, pragmatism, positivism, and realism. I've listed them in order of their influence on philosophy in the first half of the century. Or, you could just as well say, I've listed them in inverse order of their influence on philosophy in the second half of the century. 
...
> It has been said that a system of philosophy is not refuted, but becomes ignored. This is true. It is equally true (and for the same reason) that a clash of systems in the philosophical drama ends not in victory and defeat, but in a changing of the scene. Put from a somewhat different point of view, the historical development of philosophy is more truly conceived as the periodic formulation of new questions, than as a series of attempted answers to an enduring body of problems. To be sure, the new questions which appear in this process can be regarded, for the most part, as revisions or reformulations of earlier issues; however, the fact of revision and reformu- lation is of the essence of the matter, making new questions out of old. Put in these terms, a system dies when the questions it seeks to answer are no longer asked; and only where the questions are the same can there be a genuine clash of answers. ([Sellars, Realism and the New Way of Words](https://philpapers.org/rec/SELRAT), 601)

Idealism became ignored.

To get a sense of how dramatically it was ignored, let's look at some of the most prominent authors in this topic, and look at their later influence. I'd normally measure influence by citations, but citation practices have changed enough over time that this is a poor measure. Instead, for authors with distinctive enough names, I'll look at how frequently their names are used. So let's start with the authors. These are the 20 authors with the most articles in this topic. (I'm individuating authors by name here, which is a bit sloppy since some people write under multiple names. But this gives you a rough idea of who is represented in this topic.)
...
Now let's look at how often these men (and they are all men) are mentioned in the twelve journals over the 138 years under discussion. Note that I'm counting tokens here, so sometimes a name appears tens (or even hundreds) of times in an article. Also note that since there are more journals, and longer journals, in recent years, you'd expect graphs like these to trend upwards. (Later I'll measure word usage as a proportion of all words in a year to account for this, but for now the raw numbers are more informative.)
...
```{r idealism_author_facet, fig.height = 10.2}
...
ggplot(idealist_data, aes(x = year, y = count)) + 
  ...
  labs(x = "Year", y = "Word Count", title = "Idealists in the Journals")
