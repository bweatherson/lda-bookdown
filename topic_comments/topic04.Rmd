...
```{r t04b, fig.cap=paste(the_categories$subject[jjj], "Articles in Each Journal")}
source('topic_summary_facet_graph.R')
```

```{r t04c, fig.cap=the_categories$subject[jjj], fig.height= 5.2}
cat("<br>")
...
history_articles <- articles %>%
  filter(file %in% filter(relabeled_articles, topic == 4)$document)

history_word_list <- all_journals_tibble %>%
  filter(filename %in% filter(relabeled_articles, topic == 4)$document)

history_mentions <- history_word_list %>%
  group_by(word) %>%
  summarise(n1 = n_distinct(filename))


history_mentions_10 <- history_word_list %>%
  filter(wordcount > 9) %>%
  group_by(word) %>%
  summarise(n10 = n_distinct(filename))

history_mentions_50 <- history_word_list %>%
    filter(wordcount > 49) %>%
    group_by(word) %>%
    summarise(n50 = n_distinct(filename))
...
all_mention_50 <- all_journals_tibble %>%
  ...
  filter(wordcount > 49) %>%
  group_by(word) %>%
  summarise(a50 = n_distinct(filename))
