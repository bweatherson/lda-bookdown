...
```{r t34b, fig.cap=paste(the_categories$subject[jjj], "Articles in Each Journal")}
source('topic_summary_facet_graph.R')
```

```{r t34c, fig.cap=the_categories$subject[jjj], fig.height= 5.2}
cat("<br>")
...
A very small topic on the analytic/synthetic distinction. It's small because so many of the papers that you might have thought would end up in here are instead in [Topic 60](#topic60), which is all about Quinean philosophy of language. So what we get here is a combination of four overlapping discussion threads.
